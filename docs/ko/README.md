...
> 💡 **[Open Science v0.17.0 출시](https://github.com/aipoch/open-science/releases/latest)** _(마지막 업데이트: 2026년 8월 18일)_. 서명된 패키지와 CDN 폴백을 갖춘 범위 지정 스페셜리스트 마켓플레이스, 텍스트 전용 모델을 위한 영구 Vision 증거 릴레이, 컨텍스트 창 구성 분석, 세션 계획 미리보기로 렌더링되는 계획 아티팩트, 빈 대화 자리표시자 배너, 자동 생성되는 스페셜리스트 및 커넥터 ID, 영구 브라우저 해지, 마켓플레이스 설치 무결성 강화를 추가했습니다. 이와 함께 대기 중인 승인 수명 주기 복원, 세션 재개 타임아웃 갱신, 나머지 SQLite 도메인 제약 조건, 터미널 세션 삭제, 지원되지 않는 세션 파일 버전 거부를 반영했습니다. 전체 변경 내역은 [최신 릴리스 노트](https://github.com/aipoch/open-science/releases/latest)를 확인하세요.
...
| **스페셜리스트**      | 범위 지정 기능이 있는 개인 스페셜리스트 에이전트 프로필, 메인 에이전트에서 실행 중 즉시 인계, 대화식 사용자 지정, 패키지 가져오기/내보내기, 불변 호출 ID, 검증 가능한 재정의를 포함한 이름 기반 ID 생성, 서명 검증·공식 및 사용자 승인 GitHub 소스·CDN 폴백·다운로드 진행률·가져오기 시 스킬 충돌 해결을 제공하는 범위 지정 스페셜리스트 마켓플레이스.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| **실행**              | 영구 코드/출력 기록이 있는 Python·R·REPL 제어 영역 커널, 같은 실행 기록에 저장되는 무상태 셸 명령, 에이전트 평가용 제한된 REPL 추론, 오프라인 프로비저닝을 지원하는 앱 관리 환경, 사용자 Python·R 인터프리터, 추가 실행 대상인 원격 SSH 컴퓨팅 호스트, 에이전트와 공유하는 사용자 터미널, 런타임 환경별 읽기 전용 설치 패키지 목록, 에이전트 파일 검사용 Notebook 아티팩트 읽기. 외부 R 런타임의 패키지 관리는 수동입니다.                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| **스킬**              | **18개의 주요** 기본 스킬, 불변 소문자 하이픈 이름을 갖는 개인 스킬, 세션 내 자연어 대화를 통한 생성, 완료된 대화 턴에서 스킬로 저장, 외부 패키지 검증을 포함한 직접 사용자 스킬 폴더, 소스·상태·텍스트 필터가 있는 일괄 활성화/비활성화 관리, 패키지 업로드, 인증된 GitHub 미리보기/가져오기, 후보 미리보기가 있는 전역 설치 스킬 가져오기, 첨부 파일이나 GitHub URL에서 에이전트가 요청한 패키지 가져오기, 구조화 검증을 포함한 camelCase Host JavaScript API, 활성화/비활성화 제어, 세션의 `/` 명시 선택.                                                                                                                                                                                                                                                                                                                                                 |
| **커넥터**            | 런타임 상태 및 복구 화면을 갖춘 **24개의 기본** 연구 커넥터, 편집 가능한 표시 이름과 분리된 불변 소문자 호출 이름을 쓰는 로컬/원격 MCP 커넥터, 검증 가능한 재정의가 있는 이름 기반 로컬 ID, 연락처 메타데이터, 커넥터/도구별 권한.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| **배포 및 지원**      | macOS, Windows, Linux 설치 프로그램, 환경·에이전트 런타임·모델 제공업체·Notebook 런타임·데이터 위치를 위한 간결한 최초 실행 마법사, 눈에 띄는 업데이트 알림이 포함된 업데이트 안내, 로컬 진단, 커뮤니티 링크.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
