...
> 💡 **[Open Science v0.20.0 已发布](https://github.com/aipoch/open-science/releases/latest)** _（最后更新于 2026 年 8 月）_。Open Science v0.20.0 新增编辑器中的会话引用（`#`）、侧边对话建议注入正在运行的主回合、长文本粘贴转换为可恢复的附件、笔记本包安装进度、全局搜索的会话编号查找、设置中合并的场景模型卡片，以及更简洁的市场浏览；同时带来重要的长会话性能优化（有界且索引化的转录渲染、笔记本历史渐进加载、更快的启动、缓存的使用量面板）、本地化的更新弹窗发行说明，并修复大量笔记本、工作区、溯源、连接器与提供商问题。详情请查看[最新发行说明](https://github.com/aipoch/open-science/releases/latest)。
