...
> 💡 **[AIPOCH Open-Science v0.28.0 已发布](https://github.com/aipoch/open-science/releases/latest)** _（最后更新于 2026 年 9 月）_。AIPOCH Open-Science v0.28.0 让已捕获的结果可验证：在隔离环境中根据封存配方重放产物版本，并通过字节级精确与科学比对规则将复现的文件与原始文件比较。分类全局搜索覆盖项目、会话、消息正文、上传与生成的文件以及文献库，并提供上下文预览。Windows 新增可选启用的 WSL2 Bash 预览，托管的 Codex 运行时提供经过测试的就地升级，模型输出中的远程媒体仅在显式激活后加载，且远程模型端点必须使用 HTTPS。详情请查看[最新发行说明](https://github.com/aipoch/open-science/releases/latest)。
