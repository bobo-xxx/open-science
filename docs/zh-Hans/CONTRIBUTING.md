...
- 使用 `react-i18next` 的 `t()` 翻译函数包装用户可见字符串。将对应翻译添加到 `src/renderer/src/locales/fr.json`（法语）、`src/renderer/src/locales/ja.json`（日语）、`src/renderer/src/locales/ko.json`（韩语）、`src/renderer/src/locales/zh-Hans.json`（简体中文）和 `src/renderer/src/locales/zh-Hant.json`（繁体中文）。使用英文文本作为翻译键。代码注释和文档保持英文。
