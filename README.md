...
> 💡 **[Open Science v0.13.1 released](https://github.com/aipoch/open-science/releases/latest)** _(last updated August 11, 2026)_. Highlights include local folder access via `@path` mentions, bounded REPL inference, project-scoped frame reads, artifact lineage access, connector runtime status and recovery, refined session sidebar rows, notebook artifact read access, and project actions with agent context — alongside hardened database migrations, workspace interaction fixes, Windows upload repair, and connector login recovery. See the [latest release notes](https://github.com/aipoch/open-science/releases/latest) for the full changelog.
