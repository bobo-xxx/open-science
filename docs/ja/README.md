...
> 💡 **[AIPOCH Open-Science v0.28.0 をリリースしました](https://github.com/aipoch/open-science/releases/latest)** _（最終更新：2026 年 9 月）_。AIPOCH Open-Science v0.28.0 は、取得済みの結果を検証可能にします。封印されたレシピからアーティファクトバージョンを隔離環境で再実行し、再現されたファイルを、バイト完全一致と科学的な比較ルールで元のファイルと比較できます。カテゴリー別のグローバル検索で、プロジェクト、セッション、メッセージ本文、アップロード済みおよび生成済みファイル、文献ライブラリを、文脈付きプレビューとともに検索できます。Windows ではオプトインの WSL2 Bash プレビューが利用可能になり、管理対象の Codex ランタイムはテスト済みのインプレースアップグレードを提供し、モデル出力内のリモートメディアは明示的に有効化した後にのみ読み込まれ、リモートのモデルエンドポイントには HTTPS が必須になります。詳細は[最新リリースノート](https://github.com/aipoch/open-science/releases/latest)を参照してください。
