...
const HOST_MESSAGE_NAMESPACED_TOOLS = [
  {
    ...
    description: `Send advisory context to the parent main conversation. If Main is running, delivery is attempted in the current turn; otherwise it is queued for the next real main user turn without waking Main. Undelivered advisories are held in memory and discarded when the application restarts. Use only after the user explicitly asks in the current Side chat turn to send, relay, forward, or tell something to Main. Do not use for ordinary Side chat questions, requests, follow-ups, or suggestions. ${HOST_MESSAGE_CONTENT_INSTRUCTION}`,
    parameters: {
      type: 'object',
      properties: {
