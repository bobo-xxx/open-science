...
> 💡 **[AIPOCH Open-Science v0.29.0 已发布](https://github.com/aipoch/open-science/releases/latest)** _（最后更新于 2026 年 9 月）_。AIPOCH Open-Science v0.29.0 让研究可携带、更易查找与启动：经过验证的技能市场支持一键与批量安装，自包含的 `.science` 会话包可在机器之间携带其证据，本地 PDF 图表提取支持表格导出，自带 R 运行时则可安装软件包并进行条件恢复。应用启动明显加快，连接器新增批量管理与公开 DOI 查询，无头 CLI 新增运行时列表、Codex 首次运行引导与终端配置文件初始化。详情请查看[最新发行说明](https://github.com/aipoch/open-science/releases/latest)。
...
| 领域                           | 核心能力                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **项目与会话**                 | 创建和组织项目，支持会话置顶、持久消息分支和侧边对话以及可编辑会话详情。可将已完成的提示编辑为持久、可选择的消息分支，而不删除原有后续路径，并恢复近期工作、草稿、对话历史和预览状态。可将某个会话导出为自包含的 `.science` 研究包，并导入到其他项目或机器——对话分支、文件版本、Notebook 记录和验证证据都会随包携带。                                                                                                                                                                                                                                                                                                                             |
| **智能体工作流**               | 在自然语言会话中获得流式响应和按用途分组的工具活动，并使用审批、停止、排队跟进、上下文压缩及重启恢复。可将已完成工作分支为新会话，并结合结构化澄清、文本/图片/PDF 批注、关联 PDF 阅读上下文、项目记忆、会话引用及需审阅的计划。分类全局搜索覆盖项目、会话、消息正文、上传与生成的文件以及文献库，并提供筛选器与上下文详情预览，可直达匹配的消息。通知、实时状态、耗时与词元详情、命令面板、来源预览和项目切换让长时间研究保持可见、可管理。                                                                                                                                                                                                       |
...
| **文献综述与参考文献管理**     | 通过 DOI、PubMed ID、arXiv ID 或文件导入参考文献——单个 PDF 通过元数据编辑器导入，或一次导入多个，带逐文件进度、重复处理与重试——并可一目了然地查看当前文献库的参考文献总数；整理合集、将参考文献关联到项目，并从回收站恢复下载的 PDF。并行检索 Europe PMC、PMC、OpenAlex、arXiv 和 Unpaywall 的开放获取全文，通过 DOI 查询出版商元数据和数据集链接，在不丢失附件或链接的情况下合并重复记录，并根据已存元数据生成带产物溯源的引用。在本地从 PDF 中提取图表——重建的图像裁剪、题注与合并表头——并可将研究表格复制或导出为 HTML、TSV 或 Markdown。                                                                                                      |
