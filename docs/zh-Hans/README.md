...
> 💡 **[Open Science v0.20.0 已发布](https://github.com/aipoch/open-science/releases/latest)** _（最后更新于 2026 年 8 月）_。Open Science v0.20.0 通过 `#` 会话引用连接不同会话，并为智能体提供仅限当前回合的只读上下文。侧边对话建议现在可在主回合运行时送达；长文本粘贴则会转换为可恢复的附件，避免挤占编辑器。有界且索引化的对话记录渲染与笔记本历史渐进加载让长会话保持流畅，而被拒绝的权限会在该回合剩余时间内持续生效。详情请查看[最新发行说明](https://github.com/aipoch/open-science/releases/latest)。
