...
> 💡 **[AIPOCH Open-Science v0.28.0 已發佈](https://github.com/aipoch/open-science/releases/latest)** _（最後更新於 2026 年 9 月）_。AIPOCH Open-Science v0.28.0 讓已擷取的結果可驗證：在隔離環境中依產物版本的密封配方重新執行，並透過位元組精確與科學比較規則，將重現的檔案與原始檔案比較。分類式全域搜尋可觸及專案、會話、訊息內文、上傳與生成的檔案及文獻庫，並提供情境預覽。Windows 新增可選用的 WSL2 Bash 預覽，受管理的 Codex 執行環境提供經測試的就地升級，模型輸出中的遠端媒體僅在明確啟用後載入，遠端模型端點則必須使用 HTTPS。詳情請參閱[最新版本說明](https://github.com/aipoch/open-science/releases/latest)。
...
| **智能體工作流程**             | 在自然語言會話中取得串流回應和依用途分組的工具活動，並使用核准、停止、排隊跟進、上下文壓縮及重新啟動復原。可將已完成工作分支為新會話，並結合結構化澄清、文字/圖片/PDF 註解、關聯 PDF 閱讀上下文、專案記憶、會話引用及需審閱的計畫。分類式全域搜尋可觸及專案、會話、訊息內文、上傳與生成的檔案及文獻庫，並附篩選器與可跳至符合訊息的情境詳細預覽。通知、即時狀態、耗時與詞元詳情、命令面板、來源預覽和專案切換讓長時間研究保持可見、可管理。                                                                                                                                        |
...
| **Python、R、Notebook 與 HPC** | 執行持久化 Python、R 和 REPL 核心以及納入同一歷史記錄的命令列指令，可使用受管理離線環境或自備直譯器。在本機工作，或透過 SSH 連接遠端主機並使用 Slurm 向 HPC 叢集提交 Notebook 工作；受保護的網路存取、加密憑證、套件與變數檢視、共用終端和漸進式歷史載入讓運算過程可控且可觀察。長時間的 Notebook、REPL 與命令列工作可在背景執行——在釋放智能體輪次的同時保留確切的執行識別、取消與溯源，並在本機執行與遠端運算作業之間自動交付結果。在 Windows x64 上，可選用的 WSL2 Bash 預覽會將 Shell 指令交由通過就緒檢查的 WSL2 環境執行。外部 R 環境的套件管理仍需手動完成。                 |
| **文獻回顧與參考文獻管理**     | 透過 DOI、PubMed ID、arXiv ID 或檔案匯入參考文獻——單一 PDF 透過中繼資料編輯器，或一次匯入多個檔案並提供逐檔進度、重複處理與重試——並可一眼掌握目前文獻庫的參考文獻總數；整理文獻集、將參考文獻關聯到專案，並可從垃圾桶復原下載的 PDF。平行檢索 Europe PMC、PMC、OpenAlex、arXiv 和 Unpaywall 的開放取用全文，在不遺失附件或連結的情況下合併重複記錄，並依已儲存中繼資料產生附產物溯源的引用。                                                                                                                                                                                       |
...
| **科學技能與資料連接器**       | 使用 **22 個精選**內建技能和 **24 個內建**研究連接器擴充研究工作流程。可透過對話或已完成工作建立技能，匯入套件和 GitHub 來源，並新增具有工具層級權限及設定匯入/匯出能力的自訂本機或遠端 MCP 連接器。核心應用程式技能一律保持啟用，讓內建入口持續可用；無介面 CLI 與 Task SDK 也能列出、檢視並啟用或停用連接器。跨資源標籤、受保護的「收藏夾」標籤和可搜尋篩選器用於組織技能、連接器和專家。                                                                                                                                                                                        |
