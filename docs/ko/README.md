...
> 💡 **[Open Science v0.20.0 출시](https://github.com/aipoch/open-science/releases/latest)** _(마지막 업데이트: 2026년 8월)_. Open Science v0.20.0은 `#` 세션 참조로 서로 다른 세션을 연결하고 에이전트에 해당 턴으로 한정된 읽기 전용 컨텍스트를 제공합니다. 사이드 채팅 조언은 이제 메인 턴이 실행되는 동안 전달되며, 긴 텍스트 붙여넣기는 작성기를 가득 채우는 대신 복원 가능한 첨부로 바뀝니다. 경계가 지정되고 인덱싱된 트랜스크립트 렌더링과 노트북 기록의 점진적 로딩으로 긴 세션의 반응성을 유지하며, 거부된 권한은 해당 턴이 끝날 때까지 확실히 유지됩니다. 자세한 내용은 [최신 릴리스 노트](https://github.com/aipoch/open-science/releases/latest)를 확인하세요.
