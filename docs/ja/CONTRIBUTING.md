...
- ユーザー向け文字列を `react-i18next` の `t()` 翻訳関数で囲みます。対応する翻訳を `src/renderer/src/locales/ja.json`（日本語）、`src/renderer/src/locales/ko.json`（韓国語）、`src/renderer/src/locales/zh-Hans.json`（簡体字中国語）、`src/renderer/src/locales/zh-Hant.json`（繁体字中国語）に追加します。英語テキストを翻訳キーに使い、コードコメントとドキュメントは英語のままにします。
