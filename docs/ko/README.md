...
> 💡 **[Open Science v0.18.0 출시](https://github.com/aipoch/open-science/releases/latest)** _(마지막 업데이트: 2026년 8월 19일)_. 즐겨찾기와 드래그 정렬을 갖춘 리소스 간 카탈로그 태그, 재설계된 스킬 관리 환경, 캐시 즉시 로딩과 기능 아이콘을 갖춘 간소화된 스페셜리스트 마켓플레이스, 스페셜리스트 외관 빠른 편집, 컴퓨팅 호스트의 SSH 비밀번호 인증, 한국어 현지화, 다국어 README 및 기여 가이드를 추가했습니다. 이와 함께 더 빠른 마켓플레이스와 커넥터 로딩, 점진적 런타임 이벤트 스트리밍, 보호된 스킬 삭제, 온보딩 데이터 루트 원자적 저장, 시작·세션·컴퓨팅·업데이터 복구 강화를 반영했습니다. 전체 변경 내역은 [최신 릴리스 노트](https://github.com/aipoch/open-science/releases/latest)를 확인하세요.
