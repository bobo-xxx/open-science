...
> 💡 **[AIPOCH Open-Science v0.28.0 출시](https://github.com/aipoch/open-science/releases/latest)** _(마지막 업데이트: 2026년 9월)_. AIPOCH Open-Science v0.28.0은 캡처된 결과를 검증 가능하게 만듭니다. 봉인된 레시피로부터 아티팩트 버전을 격리된 환경에서 재실행하고, 재현된 파일을 바이트 단위 정확 비교와 과학적 비교 규칙으로 원본과 대조할 수 있습니다. 분류별 전역 검색은 컨텍스트 미리보기와 함께 프로젝트, 세션, 메시지 본문, 업로드 및 생성 파일, 문헌 라이브러리를 아우릅니다. Windows에는 선택형 WSL2 Bash 프리뷰가 도입되었고, 관리형 Codex 런타임은 테스트된 제자리 업그레이드를 제공하며, 모델 출력의 원격 미디어는 명시적으로 활성화한 뒤에만 로드되고, 원격 모델 엔드포인트는 HTTPS를 사용해야 합니다. 자세한 내용은 [최신 릴리스 노트](https://github.com/aipoch/open-science/releases/latest)를 확인하세요.
