...
import { decodeDataPath, encodeDataPath } from '../storage/data-path-codec'
...
const encodeArtifact = (artifact: ArtifactFile, dataRoot: string): ArtifactFile => {
...
const decodeArtifact = (artifact: ArtifactFile, dataRoot: string): ArtifactFile => {
...
const encodeWorkingFile = (file: NotebookWorkingFile, dataRoot: string): NotebookWorkingFile => ({
  ...file,
  path: encodeDataPath(file.path, dataRoot) as string
})

const decodeWorkingFile = (file: NotebookWorkingFile, dataRoot: string): NotebookWorkingFile => ({
  ...file,
  path: decodeDataPath(file.path, dataRoot) as string
})

// Encodes/decodes one run record's cwd fields plus working files and historical artifacts.
const encodeRun = (run: NotebookRunRecord, dataRoot: string): NotebookRunRecord => ({
  ...run,
...
const decodeRun = (run: NotebookRunRecord, dataRoot: string): NotebookRunRecord => ({
  ...run,
