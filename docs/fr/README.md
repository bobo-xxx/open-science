...
> 💡 **[Open Science v0.19.0 est disponible](https://github.com/aipoch/open-science/releases/latest)** _(dernière mise à jour : août 2026)_. Open Science v0.19.0 ajoute un fournisseur d'abonnement xAI (Grok) OAuth, des spécialistes installés depuis la place de marché gouvernés, le suivi des dépendances entre exécutions de notebook avec analyse tree-sitter, l'envoi immédiat en cours de tour via le pilotage natif de suivi, l'aperçu des artefacts Markdown dans les messages, les limites de compaction dans la transcription, un démarrage rapide résumé d'abord grâce aux métadonnées de session indexées dans SQLite, l'aperçu en lecture seule des notebooks terminés, une rubrique À propos repensée avec centre d'aide, un raccourci d'annulation d'archivage, des limites de jetons personnalisées pour les modèles, un cycle de vie OAuth complet pour les connecteurs et le modèle deepseek-v4-flash-vision-exp ; en parallèle, les rafales d'outils IPC sont fusionnées, le drainage des événements est groupé par lots, et une longue liste de correctifs de notebook, de composeur, d'espace de travail et de fournisseurs est incluse. Consultez les [notes de version les plus récentes](https://github.com/aipoch/open-science/releases/latest) pour tous les détails.
